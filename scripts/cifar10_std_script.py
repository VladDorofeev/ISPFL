...
os.makedirs("outputs/ISP_experiments/cifar10_std/", exist_ok=True)
...
def build_command(federated_method, run_number, dynamic_clients=False):
    ...
    output_name = f"{federated_method}_cifar10_{run_number}.txt"
    if dynamic_clients:
        output_name = "ISP_" + output_name

    return params, f"outputs/ISP_experiments/cifar10_std/{output_name}"


# Run experiments
for method in FEDERATED_METHODS:
    for run_number in range(1, 6):  # Run each method 5 times
        for dynamic_clients in [False, True]:
            # Build command and output path
            params, output_path = build_command(method, run_number, dynamic_clients)
            ...
            print(
                f"Running {method} strategy. Run number {run_number}. Dynamic clients {dynamic_clients} ",
                flush=True,
            )
            print(f"Command is:\n{cmd_str}\n\n", flush=True)
