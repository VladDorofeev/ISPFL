...
os.makedirs("outputs/ISP_experiments/cifar10_not_full_comm/", exist_ok=True)
...
def build_command(federated_method, full_comm_client_amount, dynamic_clients=False):
    ...
    if dynamic_clients:
        ...
        params.append(
            f"federated_method.full_comm_client_amount={full_comm_client_amount}"
        )
        output_name = f"ISP_{federated_method}_full_comm_{full_comm_client_amount}.txt"
    else:
        params.append(f"federated_method={federated_method}")
        output_name = f"{federated_method}.txt"

    return params, f"outputs/ISP_experiments/cifar10_not_full_comm/{output_name}"


# Run experiments
for method in FEDERATED_METHODS:
    for full_comm_client_amount in [100, 80, 60, 30, 20]:
        for dynamic_clients in [True]:
            # Build command and output path
            params, output_path = build_command(
                method, full_comm_client_amount, dynamic_clients
            )
            ...
            print(
                f"Running {method} strategy. Amount clients on full communication: {full_comm_client_amount}.",
                flush=True,
            )
            print(f"Command is:\n{cmd_str}\n\n", flush=True)
