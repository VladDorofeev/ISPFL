...
os.makedirs("outputs/ISP_experiments/cifar10_sample_amount/", exist_ok=True)
...
def build_command(federated_method, num_samples, dynamic_clients=False):
    ...
    if dynamic_clients:
        ...
        params.append(f"federated_method.num_samples={num_samples}")
        output_name = f"ISP_{federated_method}_num_samples_{num_samples}.txt"
    else:
        params.append(f"federated_method={federated_method}")
        output_name = f"{federated_method}.txt"

    return params, f"outputs/ISP_experiments/cifar10_sample_amount/{output_name}"


# Run experiments
for method in FEDERATED_METHODS:
    for num_samples in [10, 7, 5, 3, 1]:
        for dynamic_clients in [True]:
            # Build command and output path
            params, output_path = build_command(method, num_samples, dynamic_clients)
            ...
            print(
                f"Running {method} strategy. In solving optimal task we sample clients {num_samples} times",
                flush=True,
            )
            print(f"Command is:\n{cmd_str}\n\n", flush=True)
