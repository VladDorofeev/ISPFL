...
os.makedirs("outputs/ISP_experiments/cifar10_step_find/", exist_ok=True)
...
def build_command(federated_method, step_find_optimal, dynamic_clients=False):
    ...
    if dynamic_clients:
        ...
        params.append(f"federated_method.step_find_optimal={step_find_optimal}")
        output_name = f"ISP_{federated_method}_step_find_{step_find_optimal}.txt"
    else:
        params.append(f"federated_method={federated_method}")
        output_name = f"{federated_method}.txt"

    return params, f"outputs/ISP_experiments/cifar10_step_find/{output_name}"


# Run experiments
for method in FEDERATED_METHODS:
    for step_find_optimal in [500, 250, 100, 50, 20]:
        for dynamic_clients in [True]:
            # Build command and output path
            params, output_path = build_command(
                method, step_find_optimal, dynamic_clients
            )
            ...
            print(
                f"Running {method} strategy. Find optimal amount client every {step_find_optimal} rounds.",
                flush=True,
            )
            print(f"Command is:\n{cmd_str}\n\n", flush=True)
